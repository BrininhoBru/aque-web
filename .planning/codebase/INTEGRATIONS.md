...
No other third-party APIs, SaaS integrations, or external SDKs detected (no Stripe, no analytics, no error-tracking SDK, no maps, no payment providers).

## Data Storage

**Databases:**
- None directly — this is a frontend-only repo. Persistence lives entirely in `aque-backend` (Postgres, per root `CLAUDE.md`).

**File Storage:**
- Local filesystem only (static assets in `public/`, served by Nginx)
...
- Custom JWT-based auth, fully implemented against `aque-backend`:
  ...
  - `src/app/core/auth/auth.interceptor.ts` - Functional `HttpInterceptorFn` attaches `Authorization: Bearer <token>` to every outgoing request; on `401` or `403` response, force-logs-out (comment notes backend uses 401 only for bad login credentials, 403 for expired/invalid/missing token on protected routes)
...
**Logs:**
- Console only (`console.error` on bootstrap failure in `src/main.ts`); no structured client-side logging

## CI/CD & Deployment

**Hosting:**
- Self-hosted: Raspberry Pi 3B running Docker Compose (Nginx container serving this app), per root `CLAUDE.md` / `fluxo-deploy-aque.md`

**CI Pipeline:**
- GitHub Actions (`.github/` directory present) - builds Docker image, pulled by the Pi in deployment

## Environment Configuration

**Required env vars:**
- None detected in the frontend build itself (no Angular environment files, no `.env` usage). API target is configuration-file-driven (`proxy.conf.json` for dev, `nginx.conf` for prod), not env-var-driven.
